...
from .serializers import PlanSerializer, ReadSubscriberSerializer, WriteSubscriberSerializer, RegisterPriceSerializer
import stripe
...
def subscribe(request):
    ...
    if serializer.is_valid():
        ...
        try:
            ...
            logger.info(f"Payment method {payment_method_id} attached to customer {customer.id}")
...
@swagger_auto_schema(
    ...
    request_body=openapi.Schema(
        ...
        properties={
            'customer_id': openapi.Schema(type=openapi.TYPE_STRING, description="Stripe customer ID"),
...
def confirm_subscription(request):
    ...
    try:
        ...
        subscription = StripeService.create_subscription(customer.id, plan.stripe_price_id)
        ...
        return Response(ReadSubscriberSerializer(subscriber).data, status=status.HTTP_201_CREATED)
...
@swagger_auto_schema(
    ...
    responses={
        200: openapi.Response(
            ...
            schema=ReadSubscriberSerializer(many=True)
        ),
    },
)
@api_view(['GET'])
...
@swagger_auto_schema(
    ...
    request_body=openapi.Schema(
        type=openapi.TYPE_OBJECT,
        properties={
            'event_type': openapi.Schema(type=openapi.TYPE_STRING, description="Type of Stripe event"),
            'customer_id': openapi.Schema(type=openapi.TYPE_STRING, description="Stripe customer ID"),
        },
...
def stripe_webhook(request):
    ...
    if event_type == 'payment_failed':
        ...
        logger.info(f"Handling subscription_schedule.completed for customer {customer_id}")
        ...
        logger.info(f"Handling subscription_schedule.updated for customer {customer_id}")
        ...
        logger.info(f"Handling subscription_schedule.expiring for customer {customer_id}")
        # Handle subscription schedule expiration
        return Response({'status': 'subscription schedule expiring'})

    elif event_type == 'customer.subscription.resumed':
        logger.info(f"Handling customer.subscription.resumed for customer {customer_id}")
        subscriber.is_active = True
        subscriber.save()
